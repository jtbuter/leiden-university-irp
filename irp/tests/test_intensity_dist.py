...
print("unique bins",
    len(set(train_bins)) / (bin_sizes[0] * bin_sizes[1] * bin_sizes[2]),
    len(set(test_bins)) / (bin_sizes[0] * bin_sizes[1] * bin_sizes[2])
)
...
print("\n".join(map(str, set(train_bins) & set(test_bins))))

# train_float_areas = [area for area, compactness in train_states]
# train_bin_areas = [area for area, compactness in train_bins]

# plt.hist(train_float_areas, bins=6)
# plt.show()
