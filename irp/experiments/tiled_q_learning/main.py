...
def evaluate(environment: Tiled, policies: List[TiledQ], steps: int = 10, ti: Optional[int] = None, return_done: Optional[bool] = False):
    if ti is None:
    ...
    for i in range(steps):
        actions = [policy.predict(state) for policy in policies]
        action = irp.utils.vote(actions)
        # action = irp.utils.vote([actions[2]])
...
coords = irp.utils.extract_subimages(np.zeros((512, 512)), s_width, s_height, overlap=0)[1]
# coords = [(208, 240)]
...
for coord in coords:
    ...
    #     t_environment.sample, t_environment.label, [t_environment.intensity_spectrum], [envs.utils.apply_threshold], return_seq=True
    # )

    # print(dissim, seq, np.nonzero(t_environment.intensity_spectrum == seq[0])[0][0])

    # print(evaluate(t_environment, policies, ti=0))

    d_sim = evaluate(t_environment, policies, ti=0)
...
print(round(sklearn.metrics.f1_score(real.astype(bool).flatten(), result.astype(bool).flatten()), 2))
