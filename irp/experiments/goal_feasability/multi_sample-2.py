...
for coord in ((256, 240), (240, 272), (240, 264), (320, 200)):
    ...
    # Create train images
    neighborhood = irp.utils.get_neighborhood(coord, image, subimage_width, subimage_height, overlap=overlap, n_size=n_size)
    ...
    subimages = np.asarray(irp.utils.extract_subimages(image, subimage_width, subimage_height, overlap=overlap)[0])
    sublabels = np.asarray(irp.utils.extract_subimages(label, subimage_width, subimage_height, overlap=overlap)[0])
    id = irp.utils.coord_to_id(coord, (512, 512), subimage_width, subimage_height, overlap=overlap)
