# Credit: https://github.com/andnp/PyFixedReps/blob/master/PyFixedReps/TileCoder.py
import numpy as np
...
def try2jit(f: T) -> T:
    try:
        ...
        if not _has_warned:
            _has_warned = True
            logging.getLogger('PyFixedReps').warn('Could not jit compile --- expect slow performance')
