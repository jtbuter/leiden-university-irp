...
def learn(env, episodes, alpha, gamma, epsilon, epsilon_decay, min_eps, learn_delay=0, write_log: Optional[Union[irp.q.Q, bool]] = True):
    ...
    if isinstance(write_log, irp.q.Q):
        ...
        model = irp.q.Q(ExpandDims(env), 0.0, tensorboard_log=os.path.join(ROOT_DIR, 'results/goal_feasability'))
        model.learn(0)

    qtable = {}

    # Initialize the Q-table
    for i in range(env.n_thresholds):
        qtable[env.reset(threshold_i=i)] = [0, 0, 0]
    ...
    for e in range(episodes):
        state = env.reset(threshold_i=None)
        done = False
        t_old = t
        dissims_ = []
